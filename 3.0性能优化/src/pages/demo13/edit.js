...
export default class Edit extends React.Component {
  ...
  render() {
    const { demo13Store, form } = this.props;
    const { getFieldDecorator } = form;
    const { editType, editItem, loading }=  demo13Store;
    return (
  ...
  onSubmit = e => {
    e.preventDefault();
    const { editType } = this.props.demo13Store;
    this.props.form.validateFields((err, values) => {
      if (!err) {
        if (editType == 'add') {
          this.props.demo13Store.add(values);
        } else {
          this.props.demo13Store.edit(values);
        }
      }
    });
  }
