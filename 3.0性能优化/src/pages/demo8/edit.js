...
export default class Edit extends React.Component {
  ...
  render() {
    const { demo8Store, form } = this.props;
    const { getFieldDecorator } = form;
    const { editType, editItem, loading }=  demo8Store;
    return (
  ...
  onSubmit = e => {
    e.preventDefault();
    const { editType } = this.props.demo8Store;
    this.props.form.validateFields((err, values) => {
      if (!err) {
        if (editType == 'add') {
          this.props.demo8Store.add(values);
        } else {
          this.props.demo8Store.edit(values);
        }
      }
    });
  }
