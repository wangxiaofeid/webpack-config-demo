...
export default class Edit extends React.Component {
  ...
  render() {
    const { demo15Store, form } = this.props;
    const { getFieldDecorator } = form;
    const { editType, editItem, loading }=  demo15Store;
    return (
  ...
  onSubmit = e => {
    e.preventDefault();
    const { editType } = this.props.demo15Store;
    this.props.form.validateFields((err, values) => {
      if (!err) {
        if (editType == 'add') {
          this.props.demo15Store.add(values);
        } else {
          this.props.demo15Store.edit(values);
        }
      }
    });
  }
