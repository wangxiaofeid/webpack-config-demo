...
export default class Edit extends React.Component {
  ...
  render() {
    const { demo11Store, form } = this.props;
    const { getFieldDecorator } = form;
    const { editType, editItem, loading }=  demo11Store;
    return (
  ...
  onSubmit = e => {
    e.preventDefault();
    const { editType } = this.props.demo11Store;
    this.props.form.validateFields((err, values) => {
      if (!err) {
        if (editType == 'add') {
          this.props.demo11Store.add(values);
        } else {
          this.props.demo11Store.edit(values);
        }
      }
    });
  }
