...
export default class Edit extends React.Component {
  ...
  render() {
    const { demo10Store, form } = this.props;
    const { getFieldDecorator } = form;
    const { editType, editItem, loading }=  demo10Store;
    return (
  ...
  onSubmit = e => {
    e.preventDefault();
    const { editType } = this.props.demo10Store;
    this.props.form.validateFields((err, values) => {
      if (!err) {
        if (editType == 'add') {
          this.props.demo10Store.add(values);
        } else {
          this.props.demo10Store.edit(values);
        }
      }
    });
  }
