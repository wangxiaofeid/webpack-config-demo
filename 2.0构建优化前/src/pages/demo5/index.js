...
export default class Demo5 extends React.Component{
  ...
  renderTable = () => {
    const { pageSize, currentPage, total, dataList, rowKey } = this.props.demo5Store;
    return (
      <Table 
      ...
      >
        ...
        <Column title="操作" dataIndex="actions" key="actions" render={(text, item) => {
          return <span className="action-btns">
            ...
            <a href="javascript:;" onClick={() => { 
              this.props.demo5Store.delete(item[rowKey])
            }}><Icon type="delete"/></a>
          </span>
        }}/>
      </Table>
  ...
  render() {
    const { showEdit, loading } = this.props.demo5Store;
    return <Spin spinning={loading} className="demo5">
      <div className={cn('page', { 'hidden': showEdit })}>
        <div className="page-form">
          <div className="new-btns">
            <Button type="primary" onClick={this.create}>新建</Button>
          </div>
          <Demo5SearchForm submit={this.submit}/>
        </div>
  ...
  submit = (values) => {
    const { pageSize } = this.props.demo5Store;
    this.props.demo5Store.search(1, pageSize, values);
  }

  pageOnChange = (current) => {
    const { searchForm, pageSize } = this.props.demo5Store;
    this.props.demo5Store.search(current, pageSize, searchForm)
  }

  showSizeChange = (current, pageSize) => {
    const { searchForm } = this.props.demo5Store;
    this.props.demo5Store.search(1, pageSize, searchForm)
  }
}

@Form.create()
class Demo5SearchForm extends React.Component{
  onSubmit = e => {
    e.preventDefault();
