...
export default class Edit extends React.Component {
  ...
  render() {
    const { demo6Store, form } = this.props;
    const { getFieldDecorator } = form;
    const { editType, editItem, loading }=  demo6Store;
    return (
  ...
  onSubmit = e => {
    e.preventDefault();
    const { editType } = this.props.demo6Store;
    this.props.form.validateFields((err, values) => {
      if (!err) {
        if (editType == 'add') {
          this.props.demo6Store.add(values);
        } else {
          this.props.demo6Store.edit(values);
        }
      }
    });
  }
