...
export default class Demo10 extends React.Component{
  ...
  renderTable = () => {
    const { pageSize, currentPage, total, dataList, rowKey } = this.props.demo10Store;
    return (
      <Table 
        ...
        <Column title="操作" dataIndex="actions" key="actions" render={(text, item) => {
          return <span className="action-btns">
            <a href="javascript:;" onClick={() => { 
              this.props.demo10Store.changeAttr({
                editType: 'edit',
                editItem: item,
                showEdit: true
              }) 
            ...
            <a href="javascript:;" onClick={() => { 
              this.props.demo10Store.delete(item[rowKey])
            }}><Icon type="delete"/></a>
          </span>
        }}/>
      </Table>
  ...
  render() {
    const { showEdit, loading } = this.props.demo10Store;
    return <Spin spinning={loading} className="demo10">
      <div className={cn('page', { 'hidden': showEdit })}>
        <div className="page-form">
          <div className="new-btns">
            <Button type="primary" onClick={this.create}>新建</Button>
          </div>
          <Demo10SearchForm submit={this.submit}/>
        </div>
  ...
  submit = (values) => {
    const { pageSize } = this.props.demo10Store;
    this.props.demo10Store.search(1, pageSize, values);
  }

  pageOnChange = (current) => {
    const { searchForm, pageSize } = this.props.demo10Store;
    this.props.demo10Store.search(current, pageSize, searchForm)
  }

  showSizeChange = (current, pageSize) => {
    const { searchForm } = this.props.demo10Store;
    this.props.demo10Store.search(1, pageSize, searchForm)
  }
}

@Form.create()
class Demo10SearchForm extends React.Component{
  onSubmit = e => {
    e.preventDefault();
